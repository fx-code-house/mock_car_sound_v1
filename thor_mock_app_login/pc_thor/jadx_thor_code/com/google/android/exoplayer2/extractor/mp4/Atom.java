...
abstract class Atom {
    ...
    public static int parseFullAtomFlags(int fullAtomInt) {
        return fullAtomInt & ViewCompat.MEASURED_SIZE_MASK;
    }
    ...
    static final class ContainerAtom extends Atom {
        ...
        public int getChildAtomOfTypeCount(int type) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                if (this.leafChildren.get(i2).type == type) {
                    i++;
                }
            }
            int size2 = this.containerChildren.size();
            for (int i3 = 0; i3 < size2; i3++) {
                if (this.containerChildren.get(i3).type == type) {
                    i++;
                }
            }
            return i;
    ...
    public static String getAtomTypeString(int type) {
        char c = (char) ((type >> 16) & 255);
        char c2 = (char) ((type >> 8) & 255);
        return new StringBuilder(4).append((char) ((type >> 24) & 255)).append(c).append(c2).append((char) (type & 255)).toString();
    }
}
