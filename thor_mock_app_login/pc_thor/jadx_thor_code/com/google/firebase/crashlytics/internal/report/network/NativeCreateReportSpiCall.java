...
public class NativeCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest createReportRequest, boolean z) {
        ...
        HttpRequest httpRequestApplyMultipartDataTo = applyMultipartDataTo(applyHeadersTo(getHttpRequest(), createReportRequest.googleAppId), createReportRequest.organizationId, createReportRequest.report);
        Logger.getLogger().d("Sending report to: " + getUrl());
        try {
            int iCode = httpRequestApplyMultipartDataTo.execute().code();
    ...
    private HttpRequest applyHeadersTo(HttpRequest httpRequest, String str) {
        httpRequest.header("User-Agent", AbstractSpiCall.CRASHLYTICS_USER_AGENT + CrashlyticsCore.getVersion()).header(AbstractSpiCall.HEADER_CLIENT_TYPE, AbstractSpiCall.ANDROID_CLIENT_TYPE).header(AbstractSpiCall.HEADER_CLIENT_VERSION, this.version).header(AbstractSpiCall.HEADER_GOOGLE_APP_ID, str);
        return httpRequest;
    }
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, String str, Report report) {
        ...
        for (File file : report.getFiles()) {
            ...
            } else if (file.getName().equals(FileLogger.LOGS_FILE_NAME)) {
