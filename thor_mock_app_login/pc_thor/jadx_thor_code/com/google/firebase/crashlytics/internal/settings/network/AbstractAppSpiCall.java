...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData appRequestData, boolean z) {
        ...
        HttpRequest httpRequestApplyMultipartDataTo = applyMultipartDataTo(applyHeadersTo(getHttpRequest(), appRequestData), appRequestData);
        ...
        try {
            ...
            Logger.getLogger().d(("POST".equalsIgnoreCase(httpRequestApplyMultipartDataTo.method()) ? "Create" : "Update") + " app request ID: " + httpResponseExecute.header(AbstractSpiCall.HEADER_REQUEST_ID));
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, AppRequestData appRequestData) {
        HttpRequest httpRequestPart = httpRequest.part(ORGANIZATION_ID_PARAM, appRequestData.organizationId).part(APP_IDENTIFIER_PARAM, appRequestData.appId).part(APP_NAME_PARAM, appRequestData.name).part(APP_DISPLAY_VERSION_PARAM, appRequestData.displayVersion).part(APP_BUILD_VERSION_PARAM, appRequestData.buildVersion).part(APP_SOURCE_PARAM, Integer.toString(appRequestData.source)).part(APP_MIN_SDK_VERSION_PARAM, appRequestData.minSdkVersion).part(APP_BUILT_SDK_VERSION_PARAM, appRequestData.builtSdkVersion);
