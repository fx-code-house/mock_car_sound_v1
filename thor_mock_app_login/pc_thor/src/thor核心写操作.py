...
class ThorAudioExtractor:
    ...
    def parse_encrypted_response(self, encrypted_data):
        ...
        if len(decrypted_data) >= 3:
            ...
            if cmd == 8:  # 心跳响应
                ...
                self.current_command_response = {'command': cmd, 'success': True, 'raw_data': decrypted_data,
    ...
    async def _upload_mode_a(self, file_data: bytes, sound_id: int, file_type: SoundFileType) -> bool:
        ...
        try:
            ...
            response = await self.send_command_and_wait("START_WRITE_SGU_SOUND", 38, [sound_id], crypto_type=1,
                                                        timeout=8)
            if not response:
                return False
    ...
    async def _upload_mode_b(self, file_data: bytes, sound_id: int, file_type: SoundFileType, file_name: str) -> bool:
        ...
        try:
            ...
            response = await self.send_command_and_wait("DOWNLOAD_START_FILE", 113, file_params, crypto_type=1,
                                                        timeout=8)
            if not response:
                return False
            ...
            for block_idx in range(total_blocks):
                ...
                response = await self.send_command_and_wait(f"DOWNLOAD_WRITE_BLOCK_{block_idx}", 114, block_params,
                                                            crypto_type=1, timeout=8)
                if not response:
                    return False
...
async def main():
    ...
    try:
        ...
        if not response:
            logger.info("   尝试带参数格式...")
            response = await extractor.send_command_and_wait("READ_UNIVERSAL_DATA_PARAM", 56, [0], crypto_type=1,
                                                             timeout=5)

        if not response:
            logger.info("   尝试带更多参数格式...")
            response = await extractor.send_command_and_wait("READ_UNIVERSAL_DATA_EXT", 56, [1, 0], crypto_type=1,
                                                             timeout=5)
