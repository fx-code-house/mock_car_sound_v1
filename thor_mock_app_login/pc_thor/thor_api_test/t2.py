...
class ThorClient:
    ...
    def create_encrypted_request(self, command, data_body=b''):
        ...
        print(f"🔧 创建加密请求 (命令{command}):")
        print(f"   命令+数据: {command_and_body.hex().upper()}")
...
async def main():
    ...
    try:
        ...
        try:
            ...
            print(f"📡 API服务器状态: {test_response.status_code}")
        except Exception as e:
            print(f"❌ API服务器连接失败: {e}")
            print("💡 请确保API服务器和Frida hook正在运行!")
            return
        ...
        for i in range(30):  # 30秒
            ...
            # 检查是否收到任何数据
            if len(client.response_data) > 0:
                print(f"📨 收到数据: {len(client.response_data)}字节 - {client.response_data.hex().upper()}")
                # 尝试解析
                client.try_parse_response()
        ...
        if not client.sgu_sounds_complete:
            print("⏰ 30秒超时，未收到音效列表响应")
            print(f"📊 缓冲区内容: {client.response_data.hex().upper() if client.response_data else '空'}")
