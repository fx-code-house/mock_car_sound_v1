...
class ThorClient:
    ...
    def notification_handler(self, sender, data):
        """处理BLE通知"""
        logger.info("收到原始数据:{} bytes {}".format({data.hex().upper()}, len(data)))
    ...
    def try_parse_complete_response(self):
        ...
        #         preamble_found = True
        #         start_index = i
        #         logger.info(f"🎯 找到PREAMBLE at index {i}: [0xA5, 0x5A]")
        #         break
        #
        # if not preamble_found:
        #     logger.info("❌ 未找到有效的PREAMBLE")
        #     logger.info(f"🔍 当前数据: {list(self.response_data[:min(20, len(self.response_data))])}")
        #     return

        # 从PREAMBLE开始解析
        packet_data = self.response_data
    ...
    def parse_response_packet(self, data):
        ...
        expected_preamble = 0xA55A
        if preamble != expected_preamble:
            logger.info('不符合预期的前导码{}'.format(preamble))
            return
        ...
        little_crc = struct.unpack('<H', crc_bytes)[0]

        crc_data = data[:4 + data_length]
        crc_calculated = self.crc_calculator.calculate(crc_data)

        big_crc = struct.unpack('>H', crc_bytes)[0]
        logger.info('小端 0x{:04x} {}'.format(little_crc, crc_received == little_crc))
        logger.info('大端 0x{:04x} {}'.format(big_crc, crc_received == big_crc))

        if crc_received == little_crc or crc_received == big_crc:
            print("✅ CRC验证成功!")
    ...
    def parse_hardware_response(self, data):
        ...
        logger.info('hardware_version={},firmware_version={},serial_number={}'.format(
            hardware_version,
            firmware_version,
            serial_number,
        ))
