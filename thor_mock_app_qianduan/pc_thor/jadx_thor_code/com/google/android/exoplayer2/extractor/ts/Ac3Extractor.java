...
public final class Ac3Extractor implements Extractor {
    ...
    public boolean sniff(ExtractorInput input) throws IOException {
        ...
        int i3 = i;
        while (true) {
            input.peekFully(parsableByteArray.getData(), 0, 6);
            parsableByteArray.setPosition(0);
            if (parsableByteArray.readUnsignedShort() != AC3_SYNC_WORD) {
                ...
                input.advancePeekPosition(ac3SyncframeSize - 6);
            }
        }
    }
    ...
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException {
        int i = input.read(this.sampleData.getData(), 0, MAX_SYNC_FRAME_SIZE);
        if (i == -1) {
            return -1;
        }
