...
public final class AdtsExtractor implements Extractor {
    ...
    */
    ...
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException {
        ...
        int i = input.read(this.packetBuffer.getData(), 0, 2048);
        boolean z2 = i == -1;
    ...
    private void maybeOutputSeekMap(long inputLength, boolean canUseConstantBitrateSeeking, boolean readEndOfStream) {
        ...
        boolean z = canUseConstantBitrateSeeking && this.averageFrameSize > 0;
        if (z && this.reader.getSampleDurationUs() == C.TIME_UNSET && !readEndOfStream) {
            return;
        }
        if (z && this.reader.getSampleDurationUs() != C.TIME_UNSET) {
            this.extractorOutput.seekMap(getConstantBitrateSeekMap(inputLength));
        } else {
    ...
    private void calculateAverageFrameSize(ExtractorInput input) throws IOException {
        ...
        } while (input.advancePeekPosition(bits - 6, true));
        i = i2;
        input.resetPeekPosition();
        if (i > 0) {
