...
public class DefaultDownloaderFactory implements DownloaderFactory {
    ...
    public DefaultDownloaderFactory(CacheDataSource.Factory cacheDataSourceFactory) {
        this(cacheDataSourceFactory, new CredentialManager$$ExternalSyntheticLambda0());
    }
    ...
    public Downloader createDownloader(DownloadRequest request) {
        int iInferContentTypeForUriAndMimeType = Util.inferContentTypeForUriAndMimeType(request.uri, request.mimeType);
        if (iInferContentTypeForUriAndMimeType == 0 || iInferContentTypeForUriAndMimeType == 1 || iInferContentTypeForUriAndMimeType == 2) {
            return createDownloader(request, iInferContentTypeForUriAndMimeType);
        }
    ...
    private Downloader createDownloader(DownloadRequest request, int contentType) {
        ...
        try {
            return constructor.newInstance(new MediaItem.Builder().setUri(request.uri).setStreamKeys(request.streamKeys).setCustomCacheKey(request.customCacheKey).setDrmKeySetId(request.keySetId).build(), this.cacheDataSourceFactory, this.executor);
