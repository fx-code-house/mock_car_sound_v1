...
public final class MimeTypes {
    ...
    public static boolean isText(String mimeType) {
        return "text".equals(getTopLevelType(mimeType)) || APPLICATION_CEA608.equals(mimeType) || APPLICATION_CEA708.equals(mimeType) || APPLICATION_MP4CEA608.equals(mimeType) || APPLICATION_SUBRIP.equals(mimeType) || APPLICATION_TTML.equals(mimeType) || APPLICATION_TX3G.equals(mimeType) || APPLICATION_MP4VTT.equals(mimeType) || APPLICATION_RAWCC.equals(mimeType) || APPLICATION_VOBSUB.equals(mimeType) || APPLICATION_PGS.equals(mimeType) || APPLICATION_DVBSUBS.equals(mimeType);
    }
    ...
    public static boolean allSamplesAreSyncSamples(String mimeType, String codec) {
        ...
        switch (mimeType) {
            case "audio/mp4a-latm":
                if (codec != null && (objectTypeFromMp4aRFC6381CodecString = getObjectTypeFromMp4aRFC6381CodecString(codec)) != null && (encodingForAudioObjectType = AacUtil.getEncodingForAudioObjectType(objectTypeFromMp4aRFC6381CodecString.audioObjectTypeIndication)) != 0 && encodingForAudioObjectType != 16) {
                    break;
                }
                break;
        }
    ...
    public static String getCodecsCorrespondingToMimeType(String codecs, String mimeType) {
        ...
        String[] strArrSplitCodecs = Util.splitCodecs(codecs);
        StringBuilder sb = new StringBuilder();
        for (String str : strArrSplitCodecs) {
            if (mimeType.equals(getMediaMimeType(str))) {
                if (sb.length() > 0) {
                    sb.append(",");
                }
                sb.append(str);
    ...
    public static String getMediaMimeType(String codec) {
        ...
        String lowerCase = Ascii.toLowerCase(codec.trim());
        ...
        if (!lowerCase.startsWith("mp4a")) {
            return lowerCase.startsWith("mha1") ? AUDIO_MPEGH_MHA1 : lowerCase.startsWith("mhm1") ? AUDIO_MPEGH_MHM1 : (lowerCase.startsWith("ac-3") || lowerCase.startsWith("dac3")) ? AUDIO_AC3 : (lowerCase.startsWith("ec-3") || lowerCase.startsWith("dec3")) ? AUDIO_E_AC3 : lowerCase.startsWith(Ac3Util.E_AC3_JOC_CODEC_STRING) ? AUDIO_E_AC3_JOC : (lowerCase.startsWith("ac-4") || lowerCase.startsWith("dac4")) ? AUDIO_AC4 : lowerCase.startsWith("dtsc") ? AUDIO_DTS : lowerCase.startsWith("dtse") ? AUDIO_DTS_EXPRESS : (lowerCase.startsWith("dtsh") || lowerCase.startsWith("dtsl")) ? AUDIO_DTS_HD : lowerCase.startsWith("dtsx") ? AUDIO_DTS_UHD : lowerCase.startsWith("opus") ? AUDIO_OPUS : lowerCase.startsWith("vorbis") ? AUDIO_VORBIS : lowerCase.startsWith("flac") ? AUDIO_FLAC : lowerCase.startsWith("stpp") ? APPLICATION_TTML : lowerCase.startsWith("wvtt") ? TEXT_VTT : lowerCase.contains("cea708") ? APPLICATION_CEA708 : (lowerCase.contains("eia608") || lowerCase.contains("cea608")) ? APPLICATION_CEA608 : getCustomMimeTypeForCodec(lowerCase);
        }
    ...
    public static int getEncoding(String mimeType, String codec) {
        ...
        switch (mimeType) {
            ...
            case "audio/vnd.dts":
                return 7;
            case "audio/mp4a-latm":
                if (codec == null || (objectTypeFromMp4aRFC6381CodecString = getObjectTypeFromMp4aRFC6381CodecString(codec)) == null) {
                    return 0;
                }
    ...
    static Mp4aObjectType getObjectTypeFromMp4aRFC6381CodecString(String codec) {
        ...
        try {
            return new Mp4aObjectType(Integer.parseInt(str, 16), strGroup != null ? Integer.parseInt(strGroup) : 0);
