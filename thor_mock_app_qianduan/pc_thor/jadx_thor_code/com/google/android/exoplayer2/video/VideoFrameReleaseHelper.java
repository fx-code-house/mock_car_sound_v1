...
public final class VideoFrameReleaseHelper {
    ...
    interface DisplayHelper {
        ...
        public interface Listener {
            void onDefaultDisplayChanged(Display defaultDisplay);
        }
    ...
    public void onNextFrame(long framePresentationTimeUs) {
        long j = this.pendingLastAdjustedFrameIndex;
        if (j != -1) {
            this.lastAdjustedFrameIndex = j;
            this.lastAdjustedReleaseTimeNs = this.pendingLastAdjustedReleaseTimeNs;
        }
        this.frameIndex++;
        this.frameRateEstimator.onNextFrame(framePresentationTimeUs * 1000);
        updateSurfaceMediaFrameRate();
    }
    ...
    public long adjustReleaseTime(long releaseTimeNs) {
        ...
        if (this.lastAdjustedFrameIndex == -1 || !this.frameRateEstimator.isSynced()) {
            ...
            if (adjustmentAllowed(releaseTimeNs, frameDurationNs)) {
                j = frameDurationNs;
            } else {
                resetAdjustment();
                j = releaseTimeNs;
        ...
        return j2 == C.TIME_UNSET ? j : closestVsync(j, j2, this.vsyncDurationNs) - this.vsyncOffsetNs;
    }
    ...
    */
    ...
    private static void setSurfaceFrameRateV30(Surface surface, float frameRate) {
        try {
            surface.setFrameRate(frameRate, frameRate == 0.0f ? 0 : 1);
