package com.google.android.gms.internal.icing;

import java.util.Collections;
import java.util.List;
import java.util.Map;

/* JADX INFO: Add missing generic type declarations: [FieldDescriptorType] */
/* compiled from: com.google.firebase:firebase-appindexing@@19.1.0 */
/* loaded from: classes2.dex */
final class zzfy<FieldDescriptorType> extends zzfz<FieldDescriptorType, Object> {
    zzfy(int i) {
        super(i, null);
    }

    @Override // com.google.android.gms.internal.icing.zzfz
    public final void zzai() {
        if (!isImmutable()) {
            for (int i = 0; i < zzdd(); i++) {
                Map.Entry<FieldDescriptorType, Object> entryZzaj = zzaj(i);
                if (((zzdu) entryZzaj.getKey()).zzbi()) {
                    entryZzaj.setValue(Collections.unmodifiableList((List) entryZzaj.getValue()));
                }
            }
            for (Map.Entry<FieldDescriptorType, Object> entry : zzde()) {
                if (((zzdu) entry.getKey()).zzbi()) {
                    entry.setValue(Collections.unmodifiableList((List) entry.getValue()));
                }
            }
        }
        super.zzai();
    }
}
