...
public class FirebaseCrashlytics {
    ...
    static FirebaseCrashlytics init(FirebaseApp firebaseApp, FirebaseInstallationsApi firebaseInstallationsApi, CrashlyticsNativeComponent crashlyticsNativeComponent, AnalyticsConnector analyticsConnector) {
        ...
        final ExecutorService executorServiceBuildSingleThreadExecutorService = ExecutorUtils.buildSingleThreadExecutorService("com.google.firebase.crashlytics.startup");
        final SettingsController settingsControllerRetrieveSettingsData = onboarding.retrieveSettingsData(applicationContext, firebaseApp, executorServiceBuildSingleThreadExecutorService);
        final boolean zOnPreExecute = crashlyticsCore.onPreExecute(settingsControllerRetrieveSettingsData);
        Tasks.call(executorServiceBuildSingleThreadExecutorService, new Callable<Void>() { // from class: com.google.firebase.crashlytics.FirebaseCrashlytics.1
            @Override // java.util.concurrent.Callable
            public Void call() throws Exception {
                onboarding.doOnboarding(executorServiceBuildSingleThreadExecutorService, settingsControllerRetrieveSettingsData);
                if (!zOnPreExecute) {
                    return null;
                }
                crashlyticsCore.doBackgroundInitializationAsync(settingsControllerRetrieveSettingsData);
                return null;
            }
        });
